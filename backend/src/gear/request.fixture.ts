import { IFPRequest } from '../ifp';

/** Example response from https://ironforge.pro/api/players?player=Epuration-Fairbanks */
export const request: IFPRequest = {
  gear: {
    Trinket: [
      {
        id: 13965,
        gs: '63',
        rarity: 'rare',
        name: "Blackhand's Breadth",
      },
      {
        id: 11815,
        gs: '58',
        rarity: 'rare',
        name: 'Hand of Justice',
      },
      {
        id: 20130,
        gs: '52',
        rarity: 'rare',
        name: 'Diamond Flask',
      },
      {
        id: 11122,
        gs: '50',
        rarity: 'uncommon',
        name: 'Carrot on a Stick',
      },
      {
        id: 18854,
        gs: '0',
        rarity: 'rare',
        name: 'Insignia of the Alliance',
      },
    ],
    Tabard: [
      {
        id: 5976,
        gs: '1',
        rarity: 'uncommon',
        name: 'Guild Tabard',
      },
    ],
    Shoulder: [
      {
        id: 19394,
        gs: '75',
        rarity: 'epic',
        name: 'Drake Talon Pauldrons',
      },
      {
        id: 19139,
        gs: '71',
        rarity: 'epic',
        name: 'Fireguard Shoulders',
      },
      {
        id: 16868,
        gs: '66',
        rarity: 'epic',
        name: 'Pauldrons of Might',
      },
      {
        id: 12927,
        gs: '61',
        rarity: 'rare',
        name: 'Truestrike Shoulders',
      },
      {
        id: 16733,
        gs: '60',
        rarity: 'rare',
        name: 'Spaulders of Valor',
      },
    ],
    Back: [
      {
        id: 17107,
        gs: '73',
        rarity: 'epic',
        name: "Dragon's Blood Cape",
      },
      {
        id: 19436,
        gs: '70',
        rarity: 'epic',
        name: 'Cloak of Draconic Might',
      },
      {
        id: 13340,
        gs: '63',
        rarity: 'rare',
        name: 'Cape of the Black Baron',
      },
      {
        id: 15138,
        gs: '60',
        rarity: 'rare',
        name: 'Onyxia Scale Cloak',
      },
    ],
    Shirt: [
      {
        id: 2575,
        gs: '10',
        rarity: 'uncommon',
        name: 'Red Linen Shirt',
      },
    ],
    Wrist: [
      {
        id: 19146,
        gs: '65',
        rarity: 'epic',
        name: 'Wristguards of Stability',
      },
      {
        id: 12936,
        gs: '63',
        rarity: 'rare',
        name: 'Battleborn Armbraces',
      },
    ],
    Waist: [
      {
        id: 19137,
        gs: '78',
        rarity: 'epic',
        name: 'Onslaught Girdle',
      },
      {
        id: 13142,
        gs: '63',
        rarity: 'rare',
        name: 'Brigam Girdle',
      },
      {
        id: 18529,
        gs: '61',
        rarity: 'rare',
        name: 'Elemental Plate Girdle',
      },
    ],
    Feet: [
      {
        id: 19387,
        gs: '77',
        rarity: 'epic',
        name: 'Chromatic Boots',
      },
      {
        id: 18806,
        gs: '70',
        rarity: 'epic',
        name: 'Core Forged Greaves',
      },
      {
        id: 16862,
        gs: '66',
        rarity: 'epic',
        name: 'Sabatons of Might',
      },
      {
        id: 11731,
        gs: '57',
        rarity: 'rare',
        name: 'Savage Gladiator Greaves',
      },
    ],
    Legs: [
      {
        id: 19433,
        gs: '75',
        rarity: 'epic',
        name: 'Emberweave Leggings',
      },
      {
        id: 14554,
        gs: '62',
        rarity: 'epic',
        name: 'Cloudkeeper Legplates',
      },
    ],
    Finger: [
      {
        id: 19384,
        gs: '83',
        rarity: 'epic',
        name: "Master Dragonslayer's Ring",
      },
      {
        id: 17063,
        gs: '78',
        rarity: 'epic',
        name: 'Band of Accuria',
      },
      {
        id: 18821,
        gs: '67',
        rarity: 'epic',
        name: 'Quick Strike Ring',
      },
      {
        id: 19325,
        gs: '65',
        rarity: 'epic',
        name: "Don Julio's Band",
      },
    ],
    Neck: [
      {
        id: 18404,
        gs: '74',
        rarity: 'epic',
        name: 'Onyxia Tooth Pendant',
      },
      {
        id: 19856,
        gs: '68',
        rarity: 'epic',
        name: 'The Eye of Hakkar',
      },
    ],
    Ranged: [
      {
        id: 17072,
        gs: '70',
        rarity: 'epic',
        name: 'Blastershot Launcher',
      },
      {
        id: 17069,
        gs: '69',
        rarity: 'epic',
        name: "Striker's Mark",
      },
    ],
    'Main Hand': [
      {
        id: 17075,
        gs: '74',
        rarity: 'epic',
        name: "Vis'kag the Bloodletter",
      },
      {
        id: 18832,
        gs: '70',
        rarity: 'epic',
        name: 'Brutality Blade',
      },
    ],
    Head: [
      {
        id: 16963,
        gs: '76',
        rarity: 'epic',
        name: 'Helm of Wrath',
      },
      {
        id: 12640,
        gs: '61',
        rarity: 'epic',
        name: 'Lionheart Helm',
      },
    ],
    Hands: [
      {
        id: 19143,
        gs: '69',
        rarity: 'epic',
        name: 'Flameguard Gauntlets',
      },
      {
        id: 18722,
        gs: '62',
        rarity: 'rare',
        name: 'Death Grips',
      },
    ],
    Chest: [
      {
        id: 11726,
        gs: '57',
        rarity: 'epic',
        name: 'Savage Gladiator Chain',
      },
    ],
  },
  latestGear: {
    '1002': {
      date: '2020-08-06',
      gear: {
        'Main Hand': [
          {
            id: 17075,
            gs: '74',
            rarity: 'epic',
            name: "Vis'kag the Bloodletter",
            enchantID: 1900,
            enchantName: 'Crusader',
          },
          {
            id: 18832,
            gs: '70',
            rarity: 'epic',
            name: 'Brutality Blade',
            enchantID: 1900,
            enchantName: 'Crusader',
          },
        ],
        Hands: [
          {
            id: 19143,
            gs: '69',
            rarity: 'epic',
            name: 'Flameguard Gauntlets',
            enchantID: 931,
            enchantName: '+10 Haste',
          },
        ],
        Waist: [
          {
            id: 19137,
            gs: '78',
            rarity: 'epic',
            name: 'Onslaught Girdle',
          },
        ],
        Back: [
          {
            id: 19436,
            gs: '70',
            rarity: 'epic',
            name: 'Cloak of Draconic Might',
            enchantID: 849,
            enchantName: '+3 Agility',
          },
          {
            id: 15138,
            gs: '60',
            rarity: 'rare',
            name: 'Onyxia Scale Cloak',
            enchantID: 1888,
            enchantName: '+0 All Resistances',
          },
        ],
        Trinket: [
          {
            id: 13965,
            gs: '63',
            rarity: 'rare',
            name: "Blackhand's Breadth",
          },
          {
            id: 11815,
            gs: '58',
            rarity: 'rare',
            name: 'Hand of Justice',
          },
          {
            id: 20130,
            gs: '52',
            rarity: 'rare',
            name: 'Diamond Flask',
          },
          {
            id: 18854,
            gs: '0',
            rarity: 'rare',
            name: 'Insignia of the Alliance',
          },
        ],
        Finger: [
          {
            id: 18821,
            gs: '67',
            rarity: 'epic',
            name: 'Quick Strike Ring',
          },
          {
            id: 19325,
            gs: '65',
            rarity: 'epic',
            name: "Don Julio's Band",
          },
        ],
        Legs: [
          {
            id: 14554,
            gs: '62',
            rarity: 'epic',
            name: 'Cloudkeeper Legplates',
            enchantID: 2543,
            enchantName: '+10 Haste',
          },
        ],
        Chest: [
          {
            id: 11726,
            gs: '57',
            rarity: 'epic',
            name: 'Savage Gladiator Chain',
            enchantID: 928,
            enchantName: '+<!--pts1:0:0:13824-->0<!----> All Stats',
          },
        ],
        Head: [
          {
            id: 12640,
            gs: '61',
            rarity: 'epic',
            name: 'Lionheart Helm',
            enchantID: 2543,
            enchantName: '+10 Haste',
          },
        ],
        Feet: [
          {
            id: 19387,
            gs: '77',
            rarity: 'epic',
            name: 'Chromatic Boots',
            enchantID: 911,
            enchantName: 'Minor Speed Increase',
          },
        ],
        Ranged: [
          {
            id: 17069,
            gs: '69',
            rarity: 'epic',
            name: "Striker's Mark",
          },
        ],
        Shirt: [
          {
            id: 2575,
            gs: '10',
            rarity: 'uncommon',
            name: 'Red Linen Shirt',
          },
        ],
        Neck: [
          {
            id: 19856,
            gs: '68',
            rarity: 'epic',
            name: 'The Eye of Hakkar',
          },
        ],
        Wrist: [
          {
            id: 19146,
            gs: '65',
            rarity: 'epic',
            name: 'Wristguards of Stability',
            enchantID: 1885,
            enchantName: '+9 Strength',
          },
        ],
        Shoulder: [
          {
            id: 19394,
            gs: '75',
            rarity: 'epic',
            name: 'Drake Talon Pauldrons',
            enchantID: 2606,
            enchantName: '+15 Attack Power',
          },
        ],
        Tabard: [
          {
            id: 5976,
            gs: '1',
            rarity: 'uncommon',
            name: 'Guild Tabard',
          },
        ],
      },
    },
    '1000': {
      date: '2020-07-26',
      gear: {
        Head: [
          {
            id: 12640,
            gs: '61',
            rarity: 'epic',
            name: 'Lionheart Helm',
            enchantID: 2543,
            enchantName: '+10 Haste',
          },
        ],
        Waist: [
          {
            id: 19137,
            gs: '78',
            rarity: 'epic',
            name: 'Onslaught Girdle',
          },
        ],
        Back: [
          {
            id: 13340,
            gs: '63',
            rarity: 'rare',
            name: 'Cape of the Black Baron',
            enchantID: 849,
            enchantName: '+3 Agility',
          },
          {
            id: 15138,
            gs: '60',
            rarity: 'rare',
            name: 'Onyxia Scale Cloak',
            enchantID: 1888,
            enchantName: '+0 All Resistances',
          },
        ],
        Finger: [
          {
            id: 19384,
            gs: '83',
            rarity: 'epic',
            name: "Master Dragonslayer's Ring",
          },
          {
            id: 19325,
            gs: '65',
            rarity: 'epic',
            name: "Don Julio's Band",
          },
        ],
        Trinket: [
          {
            id: 13965,
            gs: '63',
            rarity: 'rare',
            name: "Blackhand's Breadth",
          },
          {
            id: 11815,
            gs: '58',
            rarity: 'rare',
            name: 'Hand of Justice',
          },
          {
            id: 20130,
            gs: '52',
            rarity: 'rare',
            name: 'Diamond Flask',
          },
        ],
        'Main Hand': [
          {
            id: 17075,
            gs: '74',
            rarity: 'epic',
            name: "Vis'kag the Bloodletter",
            enchantID: 1900,
            enchantName: 'Crusader',
          },
          {
            id: 18832,
            gs: '70',
            rarity: 'epic',
            name: 'Brutality Blade',
            enchantID: 1900,
            enchantName: 'Crusader',
          },
        ],
        Legs: [
          {
            id: 14554,
            gs: '62',
            rarity: 'epic',
            name: 'Cloudkeeper Legplates',
            enchantID: 2543,
            enchantName: '+10 Haste',
          },
        ],
        Shoulder: [
          {
            id: 19394,
            gs: '75',
            rarity: 'epic',
            name: 'Drake Talon Pauldrons',
            enchantID: 2606,
            enchantName: '+15 Attack Power',
          },
        ],
        Wrist: [
          {
            id: 19146,
            gs: '65',
            rarity: 'epic',
            name: 'Wristguards of Stability',
            enchantID: 1885,
            enchantName: '+9 Strength',
          },
        ],
        Feet: [
          {
            id: 19387,
            gs: '77',
            rarity: 'epic',
            name: 'Chromatic Boots',
            enchantID: 911,
            enchantName: 'Minor Speed Increase',
          },
        ],
        Ranged: [
          {
            id: 17069,
            gs: '69',
            rarity: 'epic',
            name: "Striker's Mark",
          },
        ],
        Chest: [
          {
            id: 11726,
            gs: '57',
            rarity: 'epic',
            name: 'Savage Gladiator Chain',
            enchantID: 928,
            enchantName: '+<!--pts1:0:0:13824-->0<!----> All Stats',
          },
        ],
        Shirt: [
          {
            id: 2575,
            gs: '10',
            rarity: 'uncommon',
            name: 'Red Linen Shirt',
          },
        ],
        Neck: [
          {
            id: 19856,
            gs: '68',
            rarity: 'epic',
            name: 'The Eye of Hakkar',
          },
        ],
        Tabard: [
          {
            id: 5976,
            gs: '1',
            rarity: 'uncommon',
            name: 'Guild Tabard',
          },
        ],
        Hands: [
          {
            id: 19143,
            gs: '69',
            rarity: 'epic',
            name: 'Flameguard Gauntlets',
            enchantID: 931,
            enchantName: '+10 Haste',
          },
        ],
      },
    },
  },
  info: {
    id: 43791,
    name: 'Epuration',
    server: 'Fairbanks',
    spec: 'Warrior',
    race: 'Alliance',
    region: 'US',
    updated: '2020-08-06T00:00:00',
    updated_items: '2020-08-06',
  },
};
